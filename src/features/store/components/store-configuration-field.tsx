...
const configurationTypeToComponentMapping: Record<
  string,
  (props: StoreConfigurationInputProps) => React.JSX.Element
> = {
  switch: StoreConfigurationSwitch,
}

const configNameToLabelMapping = {
  enable_pos: 'Habilitar caixa',
}

export const StoreConfigurationField = ({
  configuration,
}: {
  ...
  const configurationLabel =
    configNameToLabelMapping[
      configuration.name as keyof typeof configNameToLabelMapping
    ] ?? configuration.name

  const ConfigurationInputComponent =
    configurationTypeToComponentMapping[configuration.type]
  ...
  const onChangeInput = async (value: string) => {
    ...
    updateConfiguration.mutate(
      { storeId: selectedStoreId, configurationId: configuration.id, value },
      {
        onError: () => {
          dispatchToast({
            message: `Erro ao atualizar configuração '${configurationLabel}'`,
            type: 'error',
          })
